...
public sealed class FileStore : IFileStore
{
    ...
    public async Task<Result<string>> ReadTextAsync(string relativePath, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                return VoltError.Create(ErrorCode.FileNotFound, $"File not found: {relativePath}");
            }
        ...
        {
            return VoltError.Cancelled("File read");
        }
        catch (UnauthorizedAccessException ex)
        {
            _logger.LogError(ex, "Permission denied reading {Path}", relativePath);
            return VoltError.Create(ErrorCode.PermissionDenied, "Permission denied.");
        }
        catch (IOException ex)
        {
            _logger.LogError(ex, "IO error reading {Path}", relativePath);
            return VoltError.Create(ErrorCode.StorageFailed, "Failed to read file.");
        }
    }
    ...
    public async Task<Result> WriteTextAsync(string relativePath, string content, CancellationToken ct = default)
    {
        ...
        {
            return VoltError.Cancelled("File write");
        }
        catch (UnauthorizedAccessException ex)
        {
        ...
        catch (IOException ex)
        {
            ...
            return VoltError.Create(ErrorCode.StorageFailed, "Failed to write file.");
        }
    }
    ...
    public Task<Result> DeleteAsync(string relativePath, CancellationToken ct = default)
    {
        ...
        catch (UnauthorizedAccessException ex)
        {
            ...
            return Task.FromResult<Result>(VoltError.Create(ErrorCode.PermissionDenied, "Permission denied."));
        }
        catch (IOException ex)
        {
            _logger.LogError(ex, "IO error deleting {Path}", relativePath);
            return Task.FromResult<Result>(VoltError.Create(ErrorCode.StorageFailed, "Failed to delete file."));
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        catch (UnauthorizedAccessException ex)
        {
            ...
            return Task.FromResult(Result<IReadOnlyList<string>>.Failure(
                VoltError.Create(ErrorCode.PermissionDenied, "Permission denied.")));
        }
        catch (IOException ex)
        {
            _logger.LogError(ex, "IO error listing {Path}", relativeDirectory);
            return Task.FromResult(Result<IReadOnlyList<string>>.Failure(
                VoltError.Create(ErrorCode.StorageFailed, "Failed to list files.")));
        }
    }
    ...
    private Result<string> ValidateAndResolvePath(string relativePath)
    {
        ...
        {
            return VoltError.Create(ErrorCode.InvalidArgument, "Path cannot be empty.");
        }
        ...
        if (relativePath.Contains("..") || Path.IsPathRooted(relativePath))
        {
            ...
            return VoltError.PathNotAllowed(relativePath);
        }
    ...
    private static string GetDefaultAppDataPath()
    {
        ...
        return Path.Combine(localAppData, "Volt");
    }
}
