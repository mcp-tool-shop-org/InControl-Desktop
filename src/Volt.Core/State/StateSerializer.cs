...
public static class StateSerializer
{
    ...
    public static Result<T> Deserialize<T>(string json)
    {
        try
        {
            ...
            if (result is null)
            {
                return VoltError.Create(ErrorCode.DeserializationFailed, "Deserialization returned null.");
            }
            return result;
        }
        catch (JsonException ex)
        {
            return VoltError.Create(ErrorCode.DeserializationFailed, $"Invalid JSON: {ex.Message}");
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            return VoltError.Cancelled("Deserialization");
        }
    }
