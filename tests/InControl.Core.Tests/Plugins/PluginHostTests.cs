...
public class PluginAuditLogTests
{
    ...
    {
        var log = new InMemoryPluginAuditLog();

        log.LogPluginLoaded("test-plugin", "1.0.0");

        var entries = log.GetRecentEntries();
        entries.Should().ContainSingle();
        entries[0].PluginId.Should().Be("test-plugin");
        entries[0].EventType.Should().Be(PluginAuditEventType.Loaded);
    }
    ...
    {
        var log = new InMemoryPluginAuditLog();
        log.LogPluginLoaded("plugin-1", "1.0.0");
        log.LogPluginLoaded("plugin-2", "1.0.0");
        log.LogPluginEnabled("plugin-1");

        var entries = log.GetEntriesForPlugin("plugin-1");

        entries.Should().HaveCount(2);
        entries.Should().OnlyContain(e => e.PluginId == "plugin-1");
    }
    ...
    public void Clear_RemovesAllEntries()
    {
        var log = new InMemoryPluginAuditLog();
        log.LogPluginLoaded("test-plugin", "1.0.0");
        log.LogPluginEnabled("test-plugin");

        log.Clear();

        log.GetRecentEntries().Should().BeEmpty();
    }

    [Fact]
    public void MaxEntries_TrimsOldest()
    {
        var log = new InMemoryPluginAuditLog(maxEntries: 5);

        for (int i = 0; i < 10; i++)
        {
            log.LogPluginLoaded($"plugin-{i}", "1.0.0");
        }

        var entries = log.GetRecentEntries();
        entries.Should().HaveCount(5);
        // Should have newest entries (5-9)
        entries.Should().Contain(e => e.PluginId == "plugin-9");
        entries.Should().NotContain(e => e.PluginId == "plugin-0");
    }
}
